...
@Controller(':productionId/specification')
export class ProductSpecificationController {
  ...
  @Post()
  async createSpecification(@Param('productionId', ParseIntPipe) productionId: number, @Body() dto: CreateSpecificationDto) {
    return await this.productSpecificationService.createSpecification(productionId, dto);
  }

  @Get()
  async findOneSpecification(@Param('productionId', ParseIntPipe) productionId: number) {
    return await this.productSpecificationService.findOneSpecification(productionId);
  }

  @Patch()
  async updateSpecification(@Param('productionId', ParseIntPipe) productionId: number, @Body() dto: UpdateSpecificationDto) {
    return await this.productSpecificationService.updateSpecification(productionId, dto);
  }

  @Delete()
  async removeSpecification(@Param('productionId', ParseIntPipe) productionId: number) {
    return await this.productSpecificationService.removeSpecification(productionId);
  }
}
